...
public class F {
	...
	public static String wField(String field, String data) {
		return C.wFrame + "[" + C.wField + field + C.wFrame + "] " + C.mBody
				+ data + " ";
	}

	public static String lineRule() {
		return C.cGreen + C.strike
				+ "----------------------------------------------";
	}
	...
	public static String help(String cmd, String body, Rank rank) {
		if (rank == Rank.OWNER) {
			return C.rOwner + cmd + " " + C.mBody + body + " "
					+ rank(Rank.OWNER);
		}
		if (rank == Rank.ADMIN) {
			return C.rAdmin + cmd + " " + C.mBody + body + " "
					+ rank(Rank.ADMIN);
		}
		if (rank == Rank.MOD) {
			return C.rMod + cmd + " " + C.mBody + body + " "
					+ rank(Rank.MOD);
		}
		if (rank == Rank.HELPER) {
			return C.rHelp + cmd + " " + C.mBody + body + " "
					+ rank(Rank.HELPER);
		}
		return C.rAll + cmd + " " + C.mBody + body + " " + rank(Rank.DEFAULT);
	}
	...
	public static String value(int a, String variable, String value, boolean on) {
		...
		if (on) {
			return indent + C.listTitle + variable + ": " + C.listValueOn
					+ value;
		}
